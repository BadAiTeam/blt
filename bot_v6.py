#!/usr/bin/env python3
"""
Visit Bot Python — v11.1 UPGRADE (Age Verification + 1 Article + 4 Random Footer Ads)
======================================================================================
...
# v11.0 UPGRADE — MOD 2: Minimum 4 Adsterra ad clicks per session for high CPM
MIN_AD_CLICKS_PER_SESSION = 4

# v11.0 UPGRADE — MOD 2: Optimal CPM view time (seconds on ad landing page)
# Adsterra CPM credit requires ~30+ seconds view. 30-60s is optimal for high CPM.
AD_VIEW_TIME_MIN_S = 30
...
def _humanlike_interact_with_ad_page(new_page, logger, context, request_log,
    ...
    try:
        # v11.0 UPGRADE — MOD 2: extend ad-landing scroll time for CPM view (9-18s)
        # Combined with other interactions this fills the 30-60s total view time
        hi.mixed_browse_session(duration_s=random.uniform(AD_VIEW_TIME_MIN_S * 0.3, AD_VIEW_TIME_MAX_S * 0.3))
    except Exception as e:
    ...
    # v11.0 UPGRADE — MOD 1: Random click pada halaman iklan (tab baru aktif)
    # Bot melakukan scroll + klik acak agar terlihat seperti aktivitas manusia
    try:
        random_clicks = _random_click_on_page(new_page, logger, max_clicks=random.randint(2, 4))
        if random_clicks > 0:
            logger.ok(f'   {random_clicks} random klik dilakukan di halaman iklan')
    except Exception as e:
        logger.warn(f'   Random click pada iklan gagal: {e}')

    # v11.0 UPGRADE — MOD 2: Ensure optimal CPM view time (30-60s total)
    # CRITICAL FIX: Use ad_page_enter_time parameter (passed from caller)
    # instead of broken '_ad_page_enter_time' in dir() which always evaluates
    # to 0 (local scope check, not caller scope).
    if ad_page_enter_time is not None and ad_page_enter_time > 0:
        elapsed_in_ad = time.time() - ad_page_enter_time
    else:
        elapsed_in_ad = 0
    remaining_view = max(0, random.uniform(AD_VIEW_TIME_MIN_S, AD_VIEW_TIME_MAX_S) - elapsed_in_ad)
    if remaining_view > 0:
...
def _random_click_on_page(new_page, logger, max_clicks=3):
    """
    v11.0 UPGRADE — MOD 1: Random click pada halaman iklan (tab baru).
    ...
    chosen = candidates[:max(min(max_clicks, 3), len(candidates))]
...
def run_user(pw, profile, args, ad_manager=None):
    ...
    try:
        ...
        # ============================================================
        # v11.0 UPGRADE — MOD 2: Klik MINIMAL 4 iklan Adsterra per session
